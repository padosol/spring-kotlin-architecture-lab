...
구현을 바꾸기 전에 현재 동작과 실패 정책을 테스트로 고정한다. [Step 0.5](step-00-5-domain-baseline.md)에서 정의한 도메인 규칙 중 우선순위가 높은 시나리오를 테스트 후보로 삼아, 이후 레이어 분리와 도메인 모델링을 진행해도 동작이 깨졌는지 빠르게 확인할 수 있는 안전망을 만든다.
...
- [ ] Step 0.5의 테스트 후보 중 각 도메인별 실패 케이스 1개 이상 선택
